program main
    ...
    real(wp), parameter :: PI = 3.14_wp, EPSILON_0 = 8.85e-12_wp, ELEMENTARY_CHARGE = 1.60e-19
...
contains
    ...
    subroutine main_loop
        ...
        real(wp), parameter :: STEP_SIZE_PER_DIMENSION = 1e-3_wp / real(ITERATION_COUNT, wp) + 1.e-9_wp
        ...
        do iter = 1, ITERATION_COUNT
            ...
            write(1, *) temp_force
            position1 = position1 + STEP_SIZE_PER_DIMENSION
        end do ! iter
    ...
    function get_coulomb_force1(position1, position2, charge1, charge2) result(f)
        ...
        separation_magnitude_inv_cubed = dot_product(separation, separation) ** (-3._wp / 2._wp)
        scalar_factor = charge1 * charge2 * ELEMENTARY_CHARGE * ELEMENTARY_CHARGE / (4._wp * PI * EPSILON_0)
